...
namespace BuildExeBasic.Controllers
{
    ...
    public class AccountsPayableController : ControllerBase
    {
        ...
        public AccountsPayableController(IAccountsPayableRepository accountsPayableRepository, IUserLogRepository userLogRepository, IMdHashValidator mdHashValidator)
        {
        ...
        public async Task<IActionResult> Post([FromBody] BasicSearch basicSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
                ...
                try
            {
                if (basicSearch != null)
                {
                    var product = await _accountsPayableRepository.GetForReport(basicSearch);
