...
namespace BuildExeBasic.Controllers
{
    ...
    public class BalanceSheetController : ControllerBase
    {
        ...
        public BalanceSheetController(ITrialBalanceRepository trialBalanceRepository, IUserLogRepository userLogRepository, IMdHashValidator mdHashValidator)
        {
        ...
        public async Task<IActionResult> Post([FromBody] BasicSearch basicSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
                ...
                try
            {
                if (basicSearch != null)
                {
                    var product = await _trialBalanceRepository.BalanceSheet(basicSearch);
