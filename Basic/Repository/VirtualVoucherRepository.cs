...
{
    public class VirtualVoucherRepository : IVirtualVoucherRepository
    {
        ...
        public VirtualVoucherRepository(BasicContext dbContext)
        {
            _dbContext = dbContext;
        }
        public async Task<IEnumerable<Validation>> Insert(IEnumerable<VirtualVoucher> mat)
        {
            try
            {
                ...
                var purchaseList = await _dbContext.tbl_validation.FromSqlRaw("Stpro_VirtualVoucherEntry @Id, @CompanyId, @BranchId,@json,@FinancialYearId,@UserId, @Action", materialId, CompanyId, BranchId, item, FinancialYearId, UserId, Action).ToListAsync();
                return purchaseList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<IEnumerable<Validation>> Update(IEnumerable<VirtualVoucher> mat)
        {
            try
            {
                var materialId = new SqlParameter("@Id", "0");
        ...
        public async Task<string> Getedit(int CompanyId, int Branchid, int UserId, int FinancialYearId)
        {
            try
            {
                ...
                cmd.CommandText = "dbo.Stpro_VirtualVoucherEntry";
        ...
        public async Task<string> getAccountHead(int BranchId)
        {
            try
            {
                DbCommand cmd = _dbContext.Database.GetDbConnection().CreateCommand();
