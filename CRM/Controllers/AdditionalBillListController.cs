...
namespace BuildExeServices.Controllers
{
    ...
    [ApiController]
    public class AdditionalBillListController : ControllerBase
    {
        private readonly IAdditionalBillRepository _additionalBillRepository;
        private readonly IUserLogRepository _userLogRepository;
        private readonly IMdHashValidator _mdHashValidator;

        public AdditionalBillListController(IAdditionalBillRepository additionalBillRepository, IUserLogRepository userLogRepository, IMdHashValidator mdHashValidator)
        {
        ...
        public async Task<IActionResult> Get(int companyid, int BranchId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            ...
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var product = await _additionalBillRepository.GetforEdit (companyid, BranchId);
        ...
        public async Task<IActionResult> Getbyid(int companyid, int BranchId, int UserId, int FinancialYearId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            ...
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var product = await _additionalBillRepository.GetforEdituser(companyid, BranchId, UserId, FinancialYearId);
                return new OkObjectResult(product);
            }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> Get(int companyid, int BranchId,int UserId, int FinancialYearId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var product = await  _additionalBillRepository.Getforapproval(companyid, BranchId, UserId, FinancialYearId);
            return new OkObjectResult(product);
            }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> Get(int id, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var product = await _additionalBillRepository.Getdetailst (id);
            return new OkObjectResult(product);
            }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> Post([FromBody] BillSearch billSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
                ...
                try
            {
                if (billSearch != null)
                {
                    var product = await _additionalBillRepository.GetReport(billSearch);
        ...
        public async Task<IActionResult> Put([FromBody] BillSearch billSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
                ...
                try
            {
                if (billSearch != null)
                {
                    var product = await _additionalBillRepository.Getforview (billSearch);
