...
namespace BuildExeHR.Controllers
{
    ...
    [ApiController]
    public class ContractorPaymentListController : ControllerBase
    {
        private readonly IContractorPaymentRepository _contractorPaymentRepository;
        private readonly IUserLogRepository _userLogRepository;
        private readonly IMdHashValidator _mdHashValidator;
        public ContractorPaymentListController(IContractorPaymentRepository contractorPaymentRepository, IUserLogRepository userLogRepository, IMdHashValidator mdHashValidator)
        {
        ...
        public async Task< IActionResult> Get(int companyid, int branchid, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var purchase = await _contractorPaymentRepository.GetforEdit (companyid, branchid);
        ...
        public async Task<IActionResult> Getbyuser(int companyid, int branchid, int UserId, int FinancialYearId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var purchase = await _contractorPaymentRepository.GetforEdituser(companyid, branchid, UserId, FinancialYearId);
                return new OkObjectResult(purchase);
            }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> Get(int companyid, int branchid,int UserId, int FinancialYearId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var purchase =await _contractorPaymentRepository.GetforApproval(companyid, branchid, UserId, FinancialYearId);
            return new OkObjectResult(purchase);
            }
                catch (Exception ex)
                {
        ...
        {
            {
                if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
                {
                    try
                    {
                        var purchase = await _contractorPaymentRepository.GetDetailsbyid(id);
        ...
        public async Task<IActionResult> Post([FromBody] HRSearch hRSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
                ...
                try
            {
                if (hRSearch != null)
                {
                    var product =await _contractorPaymentRepository.Getjson(hRSearch);
        ...
        public async Task<IActionResult> Put([FromBody] HRSearch hRSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
                ...
                try
            {
                if (hRSearch != null)
                {
                    var product = await _contractorPaymentRepository.Getforview(hRSearch);
                    return new OkObjectResult(product);
                }
                return new NoContentResult();
            }
