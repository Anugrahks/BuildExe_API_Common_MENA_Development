...
namespace BuildExeHR.Controllers
{
    ...
    [ApiController]
    public class StaffTAController : ControllerBase
    {
        private readonly ITAMasterRepository _attendanceMonthlyRepository;
        private readonly IUserLogRepository _userLogRepository;
        private readonly IMdHashValidator _mdHashValidator;

        public StaffTAController(ITAMasterRepository attendanceMonthlyRepository, IUserLogRepository userLogRepository, IMdHashValidator mdHashValidator)
        {
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> Post([FromBody] IEnumerable<StaffTA> attendance, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
        ...
        [HttpPost("Report")]
        [Authorize]
        public async Task<IActionResult> StaffTaReport([FromBody] HRSearch hRSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
                {
                    var result = await _attendanceMonthlyRepository.StaffTaReport(hRSearch);
                    return new OkObjectResult(result);
                }
                catch (Exception ex)
                {
        ...
        [HttpPut]
        [Authorize]
        public async Task<IActionResult> Put([FromBody] IEnumerable<StaffTA> attendance, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
