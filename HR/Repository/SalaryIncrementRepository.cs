...
{
    public class SalaryIncrementRepository : ISalaryIncrementRepository
    {
        ...
        public SalaryIncrementRepository(HRContext dbContext)
        {
            _dbContext = dbContext;
        }
        public async Task<IEnumerable<Validation>> Insert(IEnumerable<SalaryIncrement> loans)
        {
            try
            {
                ...
                var purchaseList = await _dbContext.tbl_validation.FromSqlRaw("Stpro_SalaryIncrement @Id,@CompanyId, @BranchId, @FinancialYearId,@Json, @Action", materialId, CompanyId, BranchId, FinancialYearId, item, Action).ToListAsync();
                return purchaseList;
            }
            catch (Exception ex)
            {
        ...
        public async Task Delete(int Id, int userId)
        {
            try
            {
                ...
                await _dbContext.Database.ExecuteSqlRawAsync("Stpro_SalaryIncrement @Id,@CompanyId, @BranchId, @FinancialYearId,@Json, @Action", materialId, CompanyId, BranchId, FinancialYearId, item, Action);
            }
            catch (Exception ex)
            {
                Logger.ErrorLog(this.GetType().Name, MethodBase.GetCurrentMethod().Name, ex);
        ...
        public async Task<IEnumerable<Validation>> Update(IEnumerable<SalaryIncrement> loans)
        {
            try
            {
                var materialId = new SqlParameter("@Id", "0");
        ...
        public async Task<string> GetForEdituser(int companyid, int branchid, int UserId)
        {
            try
            {
                ...
                cmd.CommandText = "dbo.Stpro_SalaryIncrement";
        ...
        public async Task<string> GetForApprovals(int companyid, int branchid, int UserId)
        {
            try
            {
                DbCommand cmd = _dbContext.Database.GetDbConnection().CreateCommand();
