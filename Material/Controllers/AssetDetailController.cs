...
namespace BuildExeMaterialServices.Controllers
{
    ...
    public class AssetDetailController : ControllerBase
    {
        ...
        public AssetDetailController(IAssetDetailRepository projectSpecificationRepository, IUserLogRepository userLogRepository, IMdHashValidator mdHashValidator)
        {
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> Post([FromBody] IEnumerable<AssetDetailEntryMaster> timeSchedulerMaster, [FromHeader] string mdhash, [FromHeader] int User)
        {
        ...
        [HttpPut]
        [Authorize]
        public async Task<IActionResult> Put([FromBody] IEnumerable<AssetDetailEntryMaster> timeSchedulerMaster, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
        ...
        [HttpGet("GetPurchaseList/{BranchId}")]
        [Authorize]
        public async Task<IActionResult> GetPurchaseList(int BranchId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
                {
                    var val = await _projectSpecificationRepository.GetPurchaseList(BranchId);
        ...
        [Authorize]
        public async Task<IActionResult> GetMaterialList(MaterialSearch materialSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
                {
                    var val = await _projectSpecificationRepository.GetMaterialList(materialSearch);
