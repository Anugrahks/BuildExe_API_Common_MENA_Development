...
namespace BuildExeMaterialServices.Controllers
{
    ...
    [ApiController]
    public class MaterialRecieveListController : ControllerBase
    {
        private readonly IMaterialReceiveRepository _materialReceiveRepository ;
        private readonly IUserLogRepository _userLogRepository;
        private readonly IMdHashValidator _mdHashValidator;
        public MaterialRecieveListController(IMaterialReceiveRepository materialReceiveRepository, IUserLogRepository userLogRepository, IMdHashValidator mdHashValidator)
        {
            _materialReceiveRepository = materialReceiveRepository;
       ...
       [Authorize]
        public async Task<IActionResult> Get(int Id, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var purchase =await _materialReceiveRepository.GetDetailWithTransferQtyid(Id);
        ...
        public async Task<IActionResult> Get(int CompanyId, int Branchid, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var purchase = await _materialReceiveRepository.GetforEdit(CompanyId, Branchid);
            return new OkObjectResult(purchase);
            }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> GetByUser(int CompanyId, int Branchid, int UserId, int FinancialYearId, int IsAsset, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var purchase = await _materialReceiveRepository.GetforEdit(CompanyId, Branchid, UserId, FinancialYearId,IsAsset);
                return new OkObjectResult(purchase);
            }
                catch (Exception ex)
                {
        ...
        [HttpGet("{CompanyId}/{Branchid}/{UserID}/{FinancialYearId}/{IsAsset}")]
        [Authorize]
        public async Task<IActionResult> Get(int CompanyId, int Branchid, int UserID, int FinancialYearId, int IsAsset, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
            {
                var purchase = await _materialReceiveRepository.GetforApproval(CompanyId, Branchid, UserID, FinancialYearId, IsAsset);
            return new OkObjectResult(purchase);
            }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> GetByUser(int CompanyId, int Branchid, int UserId, int FinancialYearId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
                {
                    var purchase = await _materialReceiveRepository.GetforEdit(CompanyId, Branchid, UserId, FinancialYearId, 0);
                    return new OkObjectResult(purchase);
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> Get(int CompanyId, int Branchid, int UserID, int FinancialYearId, [FromHeader] string mdhash, [FromHeader] int User)
        {
            if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
                {
                    var purchase = await _materialReceiveRepository.GetforApproval(CompanyId, Branchid, UserID, FinancialYearId,0);
                    return new OkObjectResult(purchase);
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> Post([FromBody] MaterialSearch materialSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
            ...
            {
                var product = await _materialReceiveRepository.GetforReport(materialSearch);
        ...
        public async Task<IActionResult> Put([FromBody] MaterialSearch materialSearch, [FromHeader] string mdhash, [FromHeader] int User)
        {
                ...
                try
            {
                if (materialSearch != null)
                {
                    var product = await _materialReceiveRepository.GetforView(materialSearch);
                    return new OkObjectResult(product);
                }
                return new NoContentResult();
            }
        ...
        [HttpGet("GetRecievecematerial/{Id}")]
         [Authorize]
        public async Task<IActionResult> GetByid(int Id, [FromHeader] string mdhash, [FromHeader] int User)
        {
             if (await _mdHashValidator.ValidateMdHashAsync(mdhash, User))
            {
                try
                {
                    var purchase = await _materialReceiveRepository.Getbalancesbyid(Id);
                    return new OkObjectResult(purchase);
                }
                catch (Exception ex)
                {
