...
namespace BuildExeMaterialServices.Repository
{
    public class StockAdjustmentRepository : IStockAdjustmentRepository
    {
        ...
        public StockAdjustmentRepository(MaterialContext dbContext)
        {
            _dbContext = dbContext;
        }
        public async Task<IEnumerable<Validation>> Insert(IEnumerable<StockAdjustment> mat)
        {
            try
            {
                ...
                var purchaseList = await _dbContext.tbl_validations.FromSqlRaw("Stpro_StockAdjustment @Id, @CompanyId, @BranchId,@json,@FinancialYearId,@UserId, @Action", materialId, CompanyId, BranchId, item, FinancialYearId, UserId, Action).ToListAsync();
                return purchaseList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<IEnumerable<Validation>> Update(IEnumerable<StockAdjustment> mat)
        {
            try
            {
                var materialId = new SqlParameter("@Id", "0");
        ...
        public async Task<string> Getedit(int CompanyId, int Branchid, int UserId, int FinancialYearId)
        {
            try
            {
                ...
                cmd.CommandText = "dbo.Stpro_StockAdjustment";
