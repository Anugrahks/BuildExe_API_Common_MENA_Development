    ...
    public class SupplierAdvanceRepository : ISupplierAdvanceRepository
    {
        ...
        public async Task<IEnumerable<Validation>> Insert(SupplierAdvance supplierAdvance)
        {
            try
            {
                ...
                var CurrencyId = new SqlParameter("@CurrencyId", supplierAdvance.CurrencyId ?? 0);
                var ExchangeRate = new SqlParameter("@ExchangeRate", supplierAdvance.ExchangeRate ?? 0);
                var LCAmount = new SqlParameter("@LCAmount", supplierAdvance.LCAmount ?? 0);

                return await _dbContext.tbl_validations.FromSqlRaw("Stpro_SupplierAdvance @Id, @PaymentDate, @SupplierId,@ProjectId,@UnitId,@BlockId,@FloorId, @PaymentMode,@PaymentBy, @PaymentNo, @AdvanceAmount, @AdvanceRecoveryBalance, @CompanyId, @BranchId, @FinancialYearId, @Narration, @WithClear,@sitemanagerId, @ApprovalStatus, @ApprovalLevel, @ApprovedBy, @ApprovedDate, @VoucherTypeId, @VoucherNumber, @IsDeleted,@chequeDate, @Action, @UserId, @IsReject,@SiteLoan, @SiteLoanAmt,@ApprovalRemarks,@RejectRemarks,@CurrencyId,@ExchangeRate,@LCAmount,@json", Id, PaymentDate, SupplierId, ProjectId, UnitId, BlockId, FloorId, PaymentMode, PaymentBy, PaymentNo, AdvanceAmount, AdvanceRecoveryBalance, CompanyId, BranchId, FinancialYearId, Narration, WithClear, sitemanagerid, ApprovalStatus, ApprovalLevel, ApprovedBy, ApprovedDate, VoucherTypeId, VoucherNumber, IsDeleted, chequeDate, Action, userId, IsReject, SiteLoan, SiteLoanAmt, ApprovalRemarks, RejectRemarks, CurrencyId, ExchangeRate, LCAmount, json).ToListAsync();
            }
            catch (Exception ex)
            {
